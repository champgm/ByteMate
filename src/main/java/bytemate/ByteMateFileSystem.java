...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = ByteMateFileSystem.MyFileSystemFactory.class)
public class ByteMateFileSystem implements GFileSystem {
	 ...
	 */
	public ByteMateFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + ByteMateFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<ByteMateFileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public ByteMateFileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			ByteMateFileSystem fs = new ByteMateFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
